				...
				<?php 
					...
					while( $featured_products->have_posts() ) { 
						...
						if( strlen( $product_title ) > 25 ) {
							$product_title = substr( $product_title, 0, 22 )."...";
						}
						...
						if( strlen( $product_excerpt ) > 60 ) {
							$product_excerpt = substr( $product_excerpt, 0, 60 )."...";
						}

						$product_price = get_field( 'price', $product_id );
						if( $product_price!="" ) {
							$product_price = get_field( 'product_currency', 'option' ).$product_price;
						}
